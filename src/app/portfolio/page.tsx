      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        {!address ? (
          ...
          <div className="space-y-4">
            ...
            <h2 className="text-xl font-semibold mt-8 mb-4">Positions</h2>
            {isLoading ? (
              <div className="text-slate-400 p-8 rounded-xl bg-slate-800/30 border border-slate-700">
                Loading positions...
              </div>
              ...
              <div className="space-y-4">
                {positions.map(({ market, yesShares, noShares }) => (
                  ...
                  >
                    <h3 className="font-semibold mb-3">{market.question}</h3>
                    <div className="flex gap-6 text-sm">
                      <span className="text-emerald-400">YES: {formatShares(yesShares)} USDC</span>
                      <span className="text-rose-400">NO: {formatShares(noShares)} USDC</span>
                    </div>
                    <p className="text-slate-500 text-xs mt-2">Status: {market.status}</p>
                  </Link>
                ))}
              </div>
            )}
