  ...
  return (
    <div className="min-h-screen relative">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <h1 className="text-3xl font-bold mb-8 text-slate-900 dark:text-white">Markets</h1>

        {isLoading ? (
          <div className="text-slate-600 dark:text-slate-400">Loading markets...</div>
        ) : isError ? (
          <div className="text-amber-600 dark:text-amber-400 p-8 rounded-xl bg-slate-100 dark:bg-slate-800/30 border border-amber-400 dark:border-amber-500/50">
          ...
          <div className="space-y-4">
            {markets.map((m: { id: string; question: string; status: string; expiration: string }) => (
              ...
              >
                <h2 className="text-lg font-semibold mb-2 text-slate-900 dark:text-white">{m.question}</h2>
